...
type RootStackParamList = {
  ...
  ResultsScreen: { directTrains: any[]; connectingTrains: any[] };
};
...
export default function HomeScreen() {
  ...
  const [searchResults, setSearchResults] = useState<SearchResults>({ directTrains: [], connectingTrains: [] });
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.238.92:8080/api/public/train-search",
        newSearch,
        {
          headers: {
      ...
      console.log("Search Results:", JSON.parse(response.data.status));
      let searchResults = JSON.parse(response.data.status);
  
      // Extract and transform the train data
      const directTrains = (searchResults.directTrains || []).map((train:any) => ({
      ...
      const connectingTrains = (searchResults.connectingTrains || []).map((connection:any) => ({
        firstTrain: {
          trainNumber: connection.firstTrain?.train_number || "",
          trainName: connection.firstTrain?.train_name || "",
          runDays: connection.firstTrain?.run_days || [],
          fromStation: connection.firstTrain?.from_station_name || "",
          toStation: connection.firstTrain?.to_station_name || "",
          departureTime: connection.firstTrain?.from_std || "",
          arrivalTime: connection.firstTrain?.to_std || "",
          classTypes: connection.firstTrain?.class_type || [],
          duration: connection.firstTrain?.duration || "",
          distance: connection.firstTrain?.distance || 0,
        },
        secondTrain: {
          trainNumber: connection.secondTrain?.train_number || "",
          trainName: connection.secondTrain?.train_name || "",
          runDays: connection.secondTrain?.run_days || [],
          fromStation: connection.secondTrain?.from_station_name || "",
          toStation: connection.secondTrain?.to_station_name || "",
          departureTime: connection.secondTrain?.from_std || "",
          arrivalTime: connection.secondTrain?.to_std || "",
          classTypes: connection.secondTrain?.class_type || [],
          duration: connection.secondTrain?.duration || "",
          distance: connection.secondTrain?.distance || 0,
        },
        connectionStation: connection.connectionStation || "",
      }));
  
      console.log("Direct Trains:", directTrains);
      console.log("Connecting Trains:", connectingTrains);
  
      // Update state with extracted and transformed train data
      setSearchResults({ directTrains, connectingTrains });
  
      // Navigate to the results screen with the train data
      navigation.navigate("ResultsScreen", { directTrains, connectingTrains });
    } catch (error) {
